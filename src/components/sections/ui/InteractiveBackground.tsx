...
export default function OriginalParticleBackground() {
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach((particle) => {
        ...
        if (distance < maxDistance) {
          ...
          particle.color = `rgba(${Math.random() * 255}, rgba(${Math.random() * 255}, rgba(${Math.random() * 255}, ${0.5 - force * 0.5})`
        }
        ...
        ctx.fillText(particle.binaryString, particle.x, particle.y)
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed top-0 left-0 w-full h-full -z-10 bg-gradient-to-br from-gray-50 to-gray-400 dark:from-gray-900 dark:to-gray-800"
    />
  )
}
